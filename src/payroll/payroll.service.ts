...
export class PayrollService {
    ...
    for (const user of employees) {
      ...
      const payroll = await this.payrollModel.findOneAndUpdate(
        { user: user._id, month, year },
        {
          ...
          status: 'DRAFT', // Reset status or keep? Requirement says "Upsert", let's update.
        },
  ...
  async findAll() {
      return this.payrollModel.find().populate('user', 'name').exec();
  }
  
  async findByMonthYear(month: number, year: number) {
      return this.payrollModel.find({ month, year }).populate('user', 'name hourly_rate').exec();
  }
}
